...
import type { ScraperInterface, ScrapeContext, RawScrapedListing, StandardListing } from './scraper.types'
...
export abstract class BaseBrowserScraper implements ScraperInterface {
  ...
  protected async newContext(): Promise<BrowserContext> {
    if (this.browser === null) {
      throw new Error(`Browser not initialized for scraper ${this.sourceId}. Call initialize() first.`)
    }
