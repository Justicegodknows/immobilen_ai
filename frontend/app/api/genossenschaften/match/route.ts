...
async function enrichMatchWithAI(
    ...
    const prompt = [
        ...
        "",
        `Genossenschaft: ${listing.source}`,
        `Listing: "${listing.title}" in ${listing.city ?? "Berlin"}, EUR ${listing.warmRentAmount ?? listing.coldRentAmount ?? 0}/month, ${listing.areaM2 ?? 0}m2`,
        "",
        `Applicant income: EUR ${tenant.monthlyNetIncomeEur}/month`,
        `Income-to-rent ratio: ${(tenant.monthlyNetIncomeEur / Math.max(Number(listing.warmRentAmount ?? listing.coldRentAmount ?? 1), 1)).toFixed(1)}x`,
